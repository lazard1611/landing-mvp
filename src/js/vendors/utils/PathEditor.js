	...
	_proto3.getString = function getString(applyTransforms, offsetX, offsetY) {
		if (applyTransforms) {
			...
			return rawPathToString(
				transformRawPath(copyRawPath(this._rawPath), 1, 0, 0, 1, m.e + (offsetX || 0), m.f + (offsetY || 0)),
			);
		}
...
PathEditor.simplifySVG = function (data, vars) {
	...
	points =
		vars.curved === false && !/[achqstvz]/gi.test(data) ? data.match(_numbersExp) : stringToRawPath(data)[0];
